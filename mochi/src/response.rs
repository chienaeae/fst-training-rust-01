// FIXME: allow: clippy bug https://github.com/rust-lang/rust-clippy/issues/6902
...
impl From<StatusCode> for EncapsulatedJson<(), ()> {
    #[inline]
    fn from(status_code: StatusCode) -> Self { Self { status_code, metadata: None, result: None } }
}
...
impl<T, M> EncapsulatedJson<T, M> {
    ...
    pub const fn empty() -> Self {
        Self { status_code: StatusCode::OK, metadata: None, result: None }
    }

    #[inline]
    #[must_use]
    pub const fn is_empty(&self) -> bool { self.result.is_none() }
    ...
    M: Serialize,
{
    fn into_response(self) -> Response {
        ...
        let body = body::boxed(body::Full::from(utils::to_json_vec(&JsonResponse {
...
impl<R> Payload<R> {
    #[must_use]
    pub fn ok(data: R) -> Self { Self::OK(Box::new(data)) }

    #[must_use]
    pub fn err(error: Error) -> Self { Self::Err(Box::new(error)) }
}

impl<R> From<Error> for Payload<R> {
    fn from(error: Error) -> Self { Self::err(error) }
}
